...
<aside id="sidebar-multi-level-sidebar"
    ...
    <div class="h-full px-3 py-4 overflow-y-auto bg-gray-100 dark:bg-gray-800">
        <div class="flex mt-2 items-center justify-center mb-2 ">
            <img src="{{ auth()->guard('customer')->user()->profile_photo ? asset('images/profile_image/' . auth()->guard('customer')->user()->profile_photo) : asset('images/profile_default/profile.png') }}" alt="logo" class="bg-neutral-200 rounded-full w-32 h-32 object-cover shadow-lg">
        </div>
        <hr class="mb-1">
            {{-- name of the cust --}}
            <div class="flex flex-col items-center space-x-2 justify-center">
                <h2 class="text-lg font-semibold dark:text-white">{{ auth()->guard('customer')->user()->company_name }}</h2>
                <p class="text-xs text-gray-500 dark:text-gray-400">({{ auth()->guard('customer')->user()->email }})</p>
            </div>
        <hr class="mt-2 mb-4">
