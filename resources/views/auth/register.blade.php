            ...
            <div class="grid md:grid-cols-2 items-center gap-4 max-w-6xl w-full">
                <div
                    ...
                    <form class="space-y-4" action="{{ route('signUp') }}" method="POST">
                        ...
                        <div>
                            <label class="text-gray-800 text-sm mb-2 block">Signup As </label>
                            <div class="relative flex items-center space-x-4">
                                <label class="flex items-center">
                                    <input type="radio" name="user_type" value="merchant" required
                                        class="text-sm text-gray-800 me-1 border @error('user_type') border-red-500 @else border-gray-300 @enderror px-4 py-3 rounded-lg outline-blue-600" @if(old('user_type') == 'merchant') checked @endif/>
                                    <span class="text-sm hover:cursor-pointer" id="merchant"
                                        onclick="document.querySelector('input[name=user_type][value=merchant]').click()">Merchant</span>
                                </label>
                                <label class="flex items-center">
                                    <input type="radio" name="user_type" value="customer" required
                                        class="text-sm text-gray-800 me-1 border @error('user_type') border-red-500 @else border-gray-300 @enderror px-4 py-3 rounded-lg outline-blue-600" @if(old('user_type') == 'customer') checked @endif/>
                                    <span class="text-sm hover:cursor-pointer" id="customer"
                                        onclick="document.querySelector('input[name=user_type][value=customer]').click()">Customer</span>
                                </label>
                            </div>
                        </div>
                        <div>
                            ...
                            <div class="relative flex items-center">
                                <input name="name" type="name" required
                                    class="w-full text-sm text-gray-800 border @error('name') border-red-500 @else border-gray-300 @enderror px-4 py-3 rounded-lg outline-blue-600"
                        ...
                        <p class="text-sm !mt-8 text-center text-gray-800">Alredy have an account ? <a
                                href="{{ route('login') }}"
